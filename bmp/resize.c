...
int main(int argc, char* argv[])
{
    ...
    if (argc != 4)
    {
        printf("Usage: ./copy number infile outfile\n");
        return 1;
    }
...
// iterate over infile's scanlines
    ...
    {
    // iterates through vertical lines
        for (int  b = 0; b < n; b++)
        {
        // iterate over pixels in scanline
            ...
            if (b < (n - 1))
            {
                fseek (inptr, -(long int )(oldWidth * sizeof(RGBTRIPLE) + padding), SEEK_CUR);
            }
    }    
}
